...
BOOST_AUTO_TEST_CASE(forward_backward) {
    ...
    auto gc45 = ellipsoid.coordinate(B45, L) >>
                project >>
                gaa::geodetic_cast >>
                project(23) >>
                project(20) >>
                gaa::geodetic_cast(ellipsoid);
    BOOST_TEST((gc45.latitude  - gaa::Latitude(B45)).value() < 1e-6, boost::test_tools::tolerance(1e-6));
    BOOST_TEST((gc45.longitude - gaa::Longitude(L)).value() < 1e-6, boost::test_tools::tolerance(1e-6));
}
...
BOOST_AUTO_TEST_CASE(bessel_forward_backward) {
    ...
    auto ri = gaa::cgcs2000.coordinate(B1, L1) >>
              gaa::bessel(S, A1) >>
