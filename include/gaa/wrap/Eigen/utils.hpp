...
GAA_EIGEN_XPR_FN((Mat), svd_inverse, 1) {
  ...
  double tolerance =
      singular_values(0) * std::max(m_1.rows(), m_1.cols()) * 1e-12;
  ...
  return svd.matrixV() * inv_singular_values.asDiagonal() *
...
GAA_EIGEN_XPR_FN((Mat), ols_solve_ALP, 2, kwargs args = {}) {
  ...
  if (args._has_weight_matrix()) {
    ...
    auto const &AtPAinv =
        cholesky_inverse(ols_design_matrix_AP(A, kw{}.weight_matrix(P)));
