...
namespace gaa
{
    ...
    public:
        _ellipsoid_gravity_access_t(_ellipsoid_data_section_t const &d)
            : m_data(d)
        {
        }
    ...
    template <class T>
    struct Geodetic_coordinate_caster
    {
    ...
    concept Convertible_To_Geodetic_Coordinate = requires {
        { Geodetic_coordinate_caster<T>::cast(std::declval<T>()) } -> std::convertible_to<Geodetic_coordinate>;
        { Geodetic_coordinate_caster<T>::cast_to(std::declval<T>(), std::declval<Ellipsoid>()) } -> std::convertible_to<Geodetic_coordinate>;
    };
    ...
    struct _geodetic_coordinate_cast_to_fn
    {
        ...
        _geodetic_coordinate_cast_to_fn(Ellipsoid const &e)
            : ellipsoid(e)
        {
        }
    ...
    template <class T>
    GAA_weak_channel((Geodetic_coordinate), (T), const &t, (_geodetic_coordinate_cast_fn), const &fn)
    {
        return fn(t);
    }

    template <class T>
    GAA_weak_channel((Geodetic_coordinate), (T), const &t, (_geodetic_coordinate_cast_to_fn), const &fn)
    {
        return fn(t);
    }
    ...
    struct _ellipsoid_project_accept_proxy_t
    {
        ...
        _ellipsoid_project_accept_proxy_t(Ellipsoid const &e)
            : ellipsoid(e)
        {
        }
    };
    ...
    public:
        Ellipsoid(data_type const &data, Identifier id) : m_data(data), m_identifier(id) {}
        ~Ellipsoid() = default;
...
#define GAA_latitude_assert(LATITUDE) gaa_assert(std::abs(LATITUDE) <= std::numbers::pi / 2.0, "Latitude(rad) out of range [-90, 90] deg")
#define GAA_longitude_assert(LONGITUDE) gaa_assert(std::abs(LONGITUDE) <= std::numbers::pi, "Latitude(rad) out of range [-180, 180] deg")
#define GAA_azimuth_assert(AZIMUTH) gaa_assert((AZIMUTH) <= std::numbers::pi * 2 && (AZIMUTH) >= 0, "Azimuth(rad) out of range [0, 360] deg")
#define GAA_distance_assert(DISTANCE) gaa_assert((DISTANCE) >= 0, "Distance(m) out of range [0, +inf]")