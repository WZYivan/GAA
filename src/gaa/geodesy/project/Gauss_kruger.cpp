...
bool Gauss_kruger_project::is_virtual() const { return this->band() == -1; }
...
Gauss_kruger_project_coordinate
Gauss_kruger_project::coordinate(double x, double y, int b,
...
int Gauss_kruger_project::band() const { return this->m_band; }
...
int Gauss_kruger_project::band_of(double lon) const {
  ...
  case Identifier::Three: {
    double rad_d3 = deg2rad(3.0), rad_d1_5 = rad_d3 / 2.0;
    return int(lon / rad_d3) + (std::fmod(lon, rad_d3) > rad_d1_5 ? 1 : 0);
    break;
  }
...
Gauss_kruger_project_coordinate
Gauss_kruger_project::eval(Geodetic_coordinate const &gc) const {
  ...
  double t = aux.t, t2 = std::pow(t, 2), t4 = std::pow(t, 4), n2 = aux.nu_2,
         n4 = std::pow(n2, 2);
  double p = rho2, p2 = std::pow(p, 2), p3 = std::pow(p, 3),
         p4 = std::pow(p, 4), p5 = std::pow(p, 5), p6 = std::pow(p, 6);
  double sinB = std::sin(B), cosB = std::cos(B), cosBp3 = std::pow(cosB, 3),
         cosBp5 = std::pow(cosB, 5);
  int band = m_band == -1 ? this->band_of(L) : m_band;
  double l_c = this->center_meridian(band),
         l_c_s = rad2sec(l_c) /*seconds below*/, l_s = rad2sec(L),
  ...
  double y = N / p * cosB * l + N / (6 * p3) * cosBp3 * (1 - t2 + n2) * l3 +
             N / (120 * p5) * cosBp5 *
                 (5 - 18 * t2 + t4 + 14 * n2 - 58 * n2 * t2) * l5;
  return this->coordinate(x, y, band, ellipsoid);
}

Geodetic_coordinate
Gauss_kruger_project::reval(Gauss_kruger_project_coordinate const &pc) const {
  ...
  double Nf3 = std::pow(Nf, 3), Nf5 = std::pow(Nf, 5);
  double cosBf = std::cos(Bf);
  double y2 = std::pow(y, 2), y3 = std::pow(y, 3), y4 = std::pow(y, 4),
         y5 = std::pow(y, 5), y6 = std::pow(y, 6);

  double B = Bf - tf / (2.0 * Mf * Nf) * y2 +
