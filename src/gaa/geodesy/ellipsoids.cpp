...
namespace gaa
{
    struct _ellipsoid_data_section_t
    {
        ...
        struct _principle_curvature_radius_coefficient_t
        {
            ...
            static _principle_curvature_radius_coefficient_t init(_geometry_property_t const &geo)
            {
        ...
        struct _quarter_arc_coefficient_t
        {
            ...
            static _quarter_arc_coefficient_t init(_principle_curvature_radius_coefficient_t const &coeff)
            {
                double
                    m0 = coeff.m0,
                    m2 = coeff.m2,
        ...
        _principle_curvature_radius_coefficient_t principle_curvature_radius_coefficient;
        _quarter_arc_coefficient_t quarter_arc_coefficient;
        _quarter_arc_bottom_coefficient_t quarter_arc_bottom_coefficient;

        _ellipsoid_data_section_t(_geometry_property_t const &geo, _gravity_property_t const &gra)
        {
            geometry_property = geo;
            gravity_property = gra;
            principle_curvature_radius_coefficient = _principle_curvature_radius_coefficient_t::init(geo);
            quarter_arc_coefficient = _quarter_arc_coefficient_t::init(principle_curvature_radius_coefficient);
            quarter_arc_bottom_coefficient = _quarter_arc_bottom_coefficient_t::init(geo);
        }

        ~_ellipsoid_data_section_t() = default;
    };

    _ellipsoid_data_section_t
        _krassovsky_data({.a = 6'378'245,
    ...
    Ellipsoid const
        krassovsky(_krassovsky_data, Ellipsoid::Identifier::Krassovsky),
    ...
    std::tuple<double, double, double> Ellipsoid::principle_curvature_radius(double lat) const
    {
        GAA_latitude_assert(lat);

        auto const &geo = this->data().geometry_property;
        double sinBp2 = std::pow(std::sin(lat), 2),
               a = geo.a,
               e1_2 = geo.e1_2,
               k = 1.0 - e1_2 * sinBp2;
    ...
    double Ellipsoid::meridian_arc_bottom_latitude(double len) const
    {
        ...
        double
            B0 = len / (geometry.a * (1 - geometry.e1_2) * coeff.a0),
            sin2B0 = std::sin(2 * B0),
            sin4B0 = std::sin(4 * B0),
            sin6B0 = std::sin(6 * B0),
            sin8B0 = std::sin(8 * B0),
            sin10B0 = std::sin(10 * B0),
            sin12B0 = std::sin(12 * B0),
