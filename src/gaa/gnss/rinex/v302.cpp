    ...
    Nav_hdr_section parse_nav_hdr(std::istream &in)
    {
        ...
        while (std::getline(in, line))
        {
            ...
            if (label.compare("RINEX VERSION / TYPE") == 0)
            {
                ...
                auto r =
                    x3::repeat(9)[x3::char_][(D(hdr.version))] >>
                    x3::repeat(11)[x3::char_] >>
                    x3::char_[(Af(hdr.file_type, file_type))] >> x3::repeat(19)[x3::char_] >>
                    x3::char_[(Af(hdr.satellite_system, satellite_system))] >> x3::repeat(19)[x3::char_];
            ...
            else if (label.compare("PGM / RUN BY / DATE") == 0)
            {
                ...
                auto r =
                    x3::repeat(20)[x3::char_][(STR(hdr.pgm))] >>
            ...
            else if (label.compare("COMMENT") == 0)
            {
                ...
                auto r =
                    x3::repeat(60)[x3::char_][comments_f];
                x3::parse(cnt.begin(), cnt.end(), r);
            }
            else if (label.compare("IONOSPHERIC CORR") == 0)
            {
                ...
                auto r =
                    x3::repeat(4)[x3::char_][(STRf(type, ionospheric_correction_type))] >> x3::char_ >>
...
#define INS_LP(LABEL) [&](auto val_) { hdr.leap_seconds.insert_or_assign(LABEL, val_); }

                auto r =
                    x3::repeat(6)[x3::char_][(Ia(INS_LP(tLS)))] >>
            ...
            {
                gaa_assert(tcl_counter != 2, "Syntax Error: More than 2 lines for label `TIME SYSTEM CORR`");
                if (tcl_counter == 0)
                {
                    tcl_counter = 1;
                    ...
                    auto r =
                        x3::repeat(4)[x3::char_][(STRf(hdr.time_corr_type, time_correction_type))] >> x3::char_ >>
                        x3::repeat(17)[x3::char_][(D(hdr.a0))] >>
                ...
                else if (tcl_counter == 1)
                {
                    ...
                    auto r =
                        x3::char_ >> x3::repeat(5)[x3::char_][(STRf(hdr.S, augmentation_system))] >> x3::char_ >>
                        x3::repeat(2)[x3::char_][(If(hdr.utc, utc_identifier))] >> x3::char_;
                    x3::parse(cnt.begin(), cnt.end(), r);
                    continue;
                }
            }
            ...
            {
                continue;
                gaa_assert(false, std::format("unknown RINEX Header label(v302): `{}`", label));
            }
        }
    ...
    Nav_dat_section_bds parse_nav_dat_bds(std::istream &in, Nav_hdr_section const &hdr)
    {
        gaa_assert(in.good(), "input stream not good");
        ...
        while (std::getline(in, line))
        {
            ...
            case 0:
            {
                auto sys_f = [&](auto const &ctx)
                {
                    ...
                    gaa_assert(sys == hdr.satellite_system, "Mismatch of satellite system in hdr and dat");
                    eph.satellite_system = sys;
                };

                auto PRN_f = [&](auto const &ctx)
                {
                    eph.PRN = x3::_attr(ctx);
                };
                ...
                auto r =
                    x3::char_[sys_f] >> x3::int_[PRN_f] >>
                    x3::char_ >> x3::int_[year_f] >>
                    x3::char_ >> x3::uint_[month_f] >>
                    x3::char_ >> x3::uint_[day_f] >>
                    x3::char_ >> x3::int_[hour_f] >>
                    x3::char_ >> x3::int_[minute_f] >>
                    x3::char_ >> x3::int_[second_f] >>
